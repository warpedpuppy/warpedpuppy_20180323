	...
	return {
	        ...
	        Main: function () {
	        	...
	        	let t1 = PIXI.loader.resources.l1.texture,
				    t2 = PIXI.loader.resources.l2.texture,
				    t3 = PIXI.loader.resources.l3.texture,
				    t4 = PIXI.loader.resources.l4.texture,
				    t5 = PIXI.loader.resources.l5.texture;
	        ...
	        next: function  () {
				...
				this.makeMaze();
				this.startOver_btn.mousedown =  this.startOver_btn.touchstart = this.startOver;
			...
			mouseMoveHandler: function (touchData) {
				let mouse = touchData.data.global,
                    mouseX = mouse.x,
                    mouseY = mouse.y;
                this.avatar.x = mouseX;
                this.avatar.y = mouseY;
			},
			xDrag: function  (e) {
				let mc = this.avatar;
				...
				let tile1XY = this.boardShell.toGlobal(new PIXI.Point(this.tileArray[1].x,this.tileArray[1].y)),
				    rect1 = new PIXI.Rectangle(tile1XY.x-50,tile1XY.y-50,this.tileWidthHeight+100, this.tileWidthHeight+100),
				    tile341XY = this.boardShell.toGlobal(new PIXI.Point(this.tileArray[341].x,this.tileArray[341].y)),
			...
			Tile: function () {
				const cont = new PIXI.Container(),
				    cover_mc = new PIXI.Graphics();
				cover_mc.beginFill(0x2E2C7C).drawRect(0,0,this.tileWidthHeight,this.tileWidthHeight).endFill();
				cont.cover_mc = cover_mc;


				let red_mc = new PIXI.Sprite(PIXI.loader.resources.brickWall.texture);
				red_mc.width = red_mc.height = this.tileWidthHeight;
				cont.red_mc = red_mc;

				let id_dt = new PIXI.Text('0',{fontFamily : '24px Arial', fill : 0xff1010, align : 'center'});
				cont.id_dt = id_dt;
			...
			makeMaze: function () {
				...
				let idNo = this.idNo,
				    ...
				    k,
				    tile;
				for (i = 0 ; i < this.boardWidthHeight; i++) {
					for (j = 0 ; j < this.boardWidthHeight; j++) {
						tile = this.Tile();
						...
						tile.red_mc.alpha = 0;
						for (k = 0; k < this.blueArray.length; k++) {
			...
			determinePath: function (mc, endTile) {
					let timerPause = this.forwardMotion * 1000;
